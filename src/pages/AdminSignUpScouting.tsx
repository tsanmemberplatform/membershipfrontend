...
const ScoutingDetails: React.FC = () => {
  // const [showDropdown, setShowDropdown] = useState(false);
  const [formData, setFormData] = useState(() => {
    const savedData = localStorage.getItem("signupInfo");
    const parsedData = savedData ? JSON.parse(savedData) : {};
    const getAdminAddInfo = localStorage.getItem("adminAdd");
    const adminAddInfo = getAdminAddInfo ? JSON.parse(getAdminAddInfo) : {};

    if (!parsedData.stateScoutCouncil) {
      return {
        scoutingRole: "",
        stateScoutCouncil: adminAddInfo.council,
        scoutDivision: "",
        scoutDistrict: "",
        troop: "",
      };
    }
    console.log(adminAddInfo);
    return {
      scoutingRole: parsedData.role || "",
      stateScoutCouncil: adminAddInfo.council || "",
